...
8. _Optional:_ Enable an broadcaster-only admin mode OBS dock view as shown below.
...
4. Add the copied URL from step 1 as a Dock URL, then change it a little bit. First, you'll need to change `file:///` to `http://absolute/` (I don't know why, it's some weird OBS thing). Second, append `?admin` to the URL. For example:

   ```
   http://absolute/Users/matthewjohnston4/TwitchTodoOverlay/twitchOverlays.htm?admin
   ```
